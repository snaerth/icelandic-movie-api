      ...
      if (!hasError) {
        ...
        // API key form
          ...
          req.flash(
            ...
            '<h2>Thank you</h2><p>' +
              user.username +
              ' account has been activated. Please use it responsibly.</p>'
          );
...
var queryMovies = function(req, res, next, collection, filePath) {
    ...
    DBService.findDocument(
      {
        $or: [
          {
            'ids.imdb': new RegExp(imdbidNumberOnly ? imdbidNumberOnly : imdbid)
          },
          { 'omdb.imdbID': imdbid }
        ]
      },
  ...
  // Query from DB after showtime
    ...
    DBService.findDocuments(
      {
        showtimes: {
          $elemMatch: { schedule: new RegExp(req.query.showtime, 'i') }
        }
      },
      collection,
      function(err, docs) {
  ...
  // Query from DB after certificate
    ...
    DBService.findDocuments(
      { 'certificateExtra.number': new RegExp(req.query.certificate, 'i') },
      collection,
      function(err, docs) {
        if (err) {
          renderResponse(
...
var queryImages = function(req, res, next, collection, filePath) {
  ...
  // Query images from DB after imdbid
    ...
    DBService.findDocument(
      { imdbid: new RegExp(imdbidNumberOnly ? imdbidNumberOnly : imdbid) },
      collection,
      function(err, docs) {
        if (err) {
          renderResponse(
      ...
      if (_underscore._.isEmpty(query)) {
        ...
        if (query.title) {
          data.filter(function(movie) {
            if (
              movie.title.toLowerCase().indexOf(query.title.toLowerCase()) > -1
            ) {
        ...
        // Filter imdb id
          ...
          data.filter(function(movie) {
            if (movie.ids) {
              ...
              if (
                id &&
                id.indexOf(imdbidNumberOnly ? imdbidNumberOnly : imdbid) > -1
              ) {
                filteredArray.push(movie);
              }
            }
          ...
          data.filter(function(movie) {
            ...
            if (movie.actors_abridged) {
              movie.actors_abridged.forEach(function(actor) {
                if (
                  actor.name.toLowerCase().indexOf(query.actor.toLowerCase()) >
                  -1
                ) {
                  match = true;
                }
            ...
            if (movie.directors_abridged) {
              movie.directors_abridged.forEach(function(director) {
                if (
                  director.name
                    .toLowerCase()
                    .indexOf(query.director.toLowerCase()) > -1
                ) {
                  match = true;
                }
              });
