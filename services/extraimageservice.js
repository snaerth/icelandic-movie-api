    ...
    if (films) {
        ...
        films.forEach(function (film) {
            ...
            if(imdbid !== null) { 
                ...
                //-------------------------------------
                promises.push(HttpService.getContent(config.themoviedburl + imdbid + '/images?api_key=' + config.themoviedbkey).then(function (data) {
                    var obj = {};
                    obj.imdbid = imdbid;
                    obj.results = JSON.parse(data);
                    extraImagesArr.push(obj);
                }).catch(function (err) {
                    logger.error().info('Error getting images for id: ' + imdbid + ' from moviedb, ErrorMessage : ' + err);
                }));
            }
        });
        //--------------------------------
        ...
        Promise.all(promises).then(function (data) {
            ...
            DBService.insertDocument(extraImagesArr, collectionName, function (err, result) {
                if (err) logger.databaseError().info('Error inserting document extra images to database, Error: ' + err);
            });
        }).catch(function (err) {
