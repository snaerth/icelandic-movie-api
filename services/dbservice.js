...
var url = DBconfig.MongoDBUrlPro;
...
var createCollection = function(db, collectionName, callback) {
    db.createCollection(collectionName, { size: 2147483648 }, function(err, collectionName) {
        if (err) {
        ...
        if (err) {
            ...
            createCollection(db, collectionName, function() {
                ...
                movies.forEach(function(movie) {
                    ...
                    collection.update(criteria, movie, { upsert: true }, function(err, result) {
                        callback(err, result);
                    });
                });
            });
...
var findDocument = function(queryObj, collectionName, callback) {
    connectToDataBase(function(err, db) {
        if (err) {
            ...
            var collection = db.collection(collectionName);
            if (queryObj) {
                collection.find(queryObj).limit(1).toArray(function(err, docs) {
                    callback(err, docs);
                    db.close();
                });
            } else {
 ...
 * @param {func} callback function to run 
 */
var updateDocument = function(queryObj, updateObj, multiObj, collectionName, callback) {
    connectToDataBase(function(err, db) {
        if (err) {
            logger.databaseError().info('Error connection to database ' + err);
        } else {
 ...
 * @param {func} callback function to run 
 */
var updatePartialDocument = function(queryObj, updateObj, collectionName, callback) {
    connectToDataBase(function(err, db) {
        if (err) {
            logger.databaseError().info('Error connection to database ' + err);
        } else {
...
var DBService = {
    ...
    updateDocument: function(queryObj, updateObj, multiObj, collectionName, callback) {
        updateDocument(queryObj, updateObj, multiObj, collectionName, callback);
    },
    updatePartialDocument: function(queryObj, updateObj, collectionName, callback) {
